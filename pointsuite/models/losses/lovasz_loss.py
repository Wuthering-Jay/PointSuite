...
class LovaszLoss(_Loss):
    def __init__(
        ...
        Args:
            ...
            class_seen: Optional list of classes seen in training (for partial class training)
            ignore_index: Label that indicates ignored pixels (does not contribute to loss)
            per_image: If True loss computed per each image and then averaged, else computed per whole batch
            loss_weight: Weight factor for the loss
        Shape
             - **preds** - torch.Tensor of shape (N, C) for point clouds
             - **batch** - Dict containing 'segment' key with ground truth labels of shape (N,)
        Reference
    ...
    def forward(self, preds: torch.Tensor, batch: Dict[str, Any]) -> torch.Tensor:
        """
        Forward pass for Lovasz loss.
        
        Args:
            preds: Predictions tensor of shape [N, C] (logits or probabilities)
            batch: Dictionary containing at least:
                - 'segment': Ground truth labels of shape [N]
                
        Returns:
            Computed Lovasz loss (scalar tensor)
        """
        # Extract ground truth labels from batch
        y_true = batch.get('class', batch.get('labels'))
        y_pred = preds
