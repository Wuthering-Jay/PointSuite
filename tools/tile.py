...
class LASProcessor:
    ...
    def process_file(self, las_file: Union[str, Path]):
        ...
        if hasattr(las_data, 'classification') and self.require_labels:
            # 如果指定了 require_labels，则只保留这些类别的点
            included_mask = np.zeros(len(las_data.points), dtype=bool)
            for label in self.require_labels:
...
def process_las_files(input_path, output_dir=None, window_size=(50.0, 50.0), 
                      ...
                      require_labels=None, label_remap=False, label_count=False,
...
if __name__ == "__main__":
    ...
    input_path=r"E:\data\DALES\dales_las\train\5080_54435.las"
    output_dir=r"E:\data\DALES\dales_las\tile1\train"
    window_size=(50., 50.)
    min_points=4096*2
    max_points=4096*16*2
    require_labels=[1,2,3,4,5]
    label_remap=True
    label_count=True
