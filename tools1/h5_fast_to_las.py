"""
快速H5到LAS转换工具 - 适配连续存储格式

支持新的快速H5格式（连续存储，无indices）

使用方式:
    # 串行处理
    python h5_fast_to_las.py <h5_file>
    
    # 并行处理（推荐）
    python h5_fast_to_las.py <h5_file> --workers 8
    
    # 只转换指定segments
    python h5_fast_to_las.py <h5_file> --segments 0-100 --workers 4
"""
            ...
            for field in available_fields:
                ...
                if h5_field in seg_group and hasattr(new_las, las_field):
                    try:
                        ...
                        print(f"Warning: Failed to set field {field}: {e}")
...
def h5_fast_to_las(
    ...
    print(f"Format: Fast H5 (continuous storage, no indices)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert Fast H5 files to LAS segments (parallel version)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Convert with default settings (serial)
  python h5_fast_to_las.py file.h5
  
  # Convert with 8 workers (parallel, recommended)
  python h5_fast_to_las.py file.h5 --workers 8
  
  # Convert only segments 0-99
  python h5_fast_to_las.py file.h5 --segments 0-99 --workers 4
  
  # Specify output directory
  python h5_fast_to_las.py file.h5 --output ./segments --workers 8
  
Note: This tool is for the new Fast H5 format (continuous storage).
      For old format (with indices), use h5_to_las_parallel.py
        """
    )
    
    parser.add_argument('h5_file', help='Input H5 file path (fast format)')
